...
一张表数据量过大，这样在日常的操作时速度非常缓慢。为了提高响应速度，实际中就是将一些不常使用的数据移动到另一张表中，而这张表就叫历史表。历史表和原来的表结构一致。这样当前表由于只使用当前的部分数据，所以性能提高，因为实际业务中很少再去查询历史表，即使查询历史表由于使用的人也少，所以他的性能还能适用。

1）表结构一致，新创建表结果跟它一样

2）历史表引发一个骨牌效应，例如购销合同，当合同主信息放到历史表中，相关的货物必须放到历史货物中，相关的附件必须放到历史附件中。

使用insert into 表 子查询时的条件：
