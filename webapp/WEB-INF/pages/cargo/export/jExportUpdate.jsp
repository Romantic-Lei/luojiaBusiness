				...
				<div>
					<table class="commonTable" cellspacing="1">
						<tr>
							...
							<td class="tableContent"><select name="state">
									<%--  <option value="0" <c:if test="${obj.state ==0 }">selected</c:if>>您的订单正在处理中</option>
			            <option value="1" <c:if test="${obj.state ==1 }">selected</c:if>>已上报，待装箱</option>
			            <option value="2" <c:if test="${obj.state ==2 }">selected</c:if>>已装箱，待委托</option>
			            <option value="3" <c:if test="${obj.state ==3 }">selected</c:if>>已委托，待发票通知</option>
			            <option value="4" <c:if test="${obj.state ==4 }">selected</c:if>>已发票，流程顺利完成!</option> --%>
									<c:if test="${obj.state ==0 }">
										<option value="0"
											<c:if test="${obj.state ==0 }">selected</c:if>>您的订单正在处理中</option>
									</c:if>
									<c:if test="${obj.state ==1 }">
										<option value="1"
											<c:if test="${obj.state ==1 }">selected</c:if>>已上报，待装箱</option>
										<option value="2">已装箱，待委托</option>
									</c:if>
									<c:if test="${obj.state ==2 }">
										<option value="1">已上报，待装箱</option>
										<option value="2"
											<c:if test="${obj.state ==2 }">selected</c:if>>已装箱，待委托</option>
										<option value="3">已委托，待发票通知</option>
									</c:if>
									<c:if test="${obj.state ==3 }">
										<option value="2">已装箱，待委托</option>
										<option value="3"
											<c:if test="${obj.state ==3 }">selected</c:if>>已委托，待发票通知</option>
										<option value="4">已发票，流程顺利完成!</option>
									</c:if>
									<c:if test="${obj.state ==4 }">
										<option value="3">已委托，待发票通知</option>
										<option value="4"
											<c:if test="${obj.state ==4 }">selected</c:if>>已发票，流程顺利完成!</option>
									</c:if>
							</select></td>
						</tr>
						<tr>
